	...
	return (
		<group position={[-1.5, 0, 0]}>
			{(() => { 
				...
				// Accordion (פתוח): רצף פלטות דקות (Tread+Riser) עם גרונג פנימי בכל פינה.
				// אין "מילוי" מתחת — התחתית נשארת פתוחה ונוצרת זיגזג כמו החלק העליון.
				const buildAccordionFlightsOpenPlates = () => {
					const out: React.ReactNode[] = [];
					...
					for (const flightIdx of flights) {
						...
						if (ftAll.length >= 2) {
							...
							if (hm > 1e-6) { fx = dx / hm; fz = dz / hm; }
								...
								fx = Math.cos(yaw0); fz = Math.sin(yaw0);
							}
						} else {
							const yaw0 = (ftAll[0].rotation[1] as number) || 0;
							fx = Math.cos(yaw0); fz = Math.sin(yaw0);
						...
						// Treads: כל אלמנט (גם פודסט) כ"פלטה" אופקית בעובי thick כלפי מטה
						for (let i = 0; i < ftAll.length; i++) {
							...
							out.push(
								<mesh
									key={`acc-tread-${flightIdx}-${i}`}
									position={[cur.position[0], yCenter, cur.position[2]]}
									rotation={[0, yaw, 0]}
									receiveShadow
								>
						...
						// Risers + Miter planes: רק בין אלמנטים עוקבים עם שינוי גובה (Rise)
						for (let i = 0; i < ftAll.length - 1; i++) {
							...
							// riser plate: עובי thick לכיוון אחורה (-forward), גובה = |rise|
							const riserCenterX = frontX - fx * tHalf;
							const riserCenterZ = frontZ - fz * tHalf;
							const yMid = (yTop + yTopN) / 2;
							out.push(
								<mesh
									key={`acc-riser-${flightIdx}-${i}`}
									position={[riserCenterX, yMid, riserCenterZ]}
									rotation={[0, yaw, 0]}
									receiveShadow
								>
							...
							// miter plane (45°) – סוגר משולש פנימי בין תחתית המדרך לבין גב הרום
							...
							// UV בסיסי כדי שהטקסטורה תופיע גם על הגרונג
							...
							out.push(
								<mesh key={`acc-miter-${flightIdx}-${i}`} geometry={g} receiveShadow>
