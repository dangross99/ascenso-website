	...
	function getTreads() {
		...
		if (pathSegments && pathSegments.length) {
			...
			if (isStraight && shape !== 'L') {
				...
				const fws0 = fws(0);
				const fws1 = fws(1);
				// סיבוב (Math.PI) רק אם: מסלול L + 180° + דגם taper – לא משפיע על Rect/Rounded
				...
				// גרם שני: L 0° = Z שלילי; L 180° = ימינה. טריז/דלתא ב־L 180° – היפוך (bodyRotate180)
			...
			for (let i = 0; i < half; i++) {
				treads.push({ position: [i * treadDepth + treadDepth / 2, i * riser, 0], rotation: [0, rotationY, 0], run: treadDepth, isLanding: false, flight: 0, axis: 'x', mirror: false, forceWallSide: fws0, bodyRotate180: false });
			}
